...
void main() {
  ...
  group('daily inputs persistence', () {
    test(
      'seeds useful presets and does not double-count embedded caffeine',
      () async {
    ...
    test(
      'schema eight migrates additively to the current schema with AI disabled by default',
      () async {
        ...
        expect(
          migrated['labDataDomains'],
          hasLength(LabDataDomain.values.length),
        );
      },
    );
  ...
  group('Lab Core analysis', () {
    test('returns honest insufficient-data evidence for a new user', () {
      ...
      expect(
        ...
        contains('Describe associations, never causation.'),
      );
    });

    test(
      'detects a preliminary caffeine association from matched sessions',
      () {
  ...
  testWidgets('daily inputs and Lab Core screens render with AI off', (
    tester,
  ) async {
    final store = AppStore();
