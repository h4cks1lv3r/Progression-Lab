...
class _IntegrationsHubScreenState extends State<IntegrationsHubScreen>
        ...
        OutlinedButton.icon(
          onPressed: _health.busy || !status.available
              ...
              : () => _run(() async {
                    ...
                    _message = '${workouts.length} health workouts reviewed. Duplicate IDs were skipped.';
                  }),
  ...
  Widget _providerCard(TrainingProvider provider) {
    ...
    final title = provider == TrainingProvider.strava ? 'STRAVA' : 'GARMIN CONNECT';
    return _Panel(
      child: Column(
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      status.connected
                          ? (status.accountName.isEmpty ? 'Connected' : status.accountName)
                          : status.state.name,
                  ...
                  child: FilledButton(
                    onPressed: _providers.busy
                        ...
                        : () => _run(() async {
                              ...
                              await _preferences.addExternalWorkouts(page.workouts);
                              _message = '${page.workouts.length} $title activities imported.';
                            }),
        ...
        children: <Widget>[
          ...
          _Panel(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                const Text('IMPORTED ACTIVITY ARCHIVE',
                  ...
                  for (final item in _preferences.externalWorkouts.reversed.take(6))
                    ListTile(
                      ...
                      trailing: item.distanceMeters == null
                          ? null
                          : Text('${(item.distanceMeters! / 1000).toStringAsFixed(1)} km'),
                    ),
                ],
              ],
            ),
  ...
  Widget _cloudTab() => _ScrollSection(
        children: <Widget>[
          ...
          FilledButton.icon(
            ...
            label: Text(_cloud.status.configured ? 'CHANGE FOLDER' : 'CHOOSE FOLDER'),
          ),
          const SizedBox(height: 12),
          SwitchListTile.adaptive(
            contentPadding: EdgeInsets.zero,
            title: const Text('AUTOMATIC CLOUD BACKUP'),
            subtitle: const Text('Upload after saved data changes. Local workout saves never wait for cloud sync.'),
          ...
          child: Column(
            children: <Widget>[
              const Align(
                ...
                child: Text('PRIVACY', style: TextStyle(fontWeight: FontWeight.w900)),
              ),
              _privacySwitch(
                'Show exact weights',
  ...
  Widget _labAndGuidesTab() => _ScrollSection(
        children: <Widget>[
          ...
          SwitchListTile.adaptive(
            ...
            subtitle: const Text('Prepare a user-triggered seven-day evidence summary. No background AI inference.'),
            value: _preferences.weeklyReviewEnabled,
            onChanged: (value) => _run(() => _preferences.setWeeklyReviewEnabled(value)),
          ),
          const SizedBox(height: 12),
  ...
  Widget _experimentCard(LabExperiment experiment) {
    final result = LabExperimentAnalyzer.analyze(experiment, widget.store.exportState());
    return _Panel(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Row(
            children: <Widget>[
              ...
              IconButton(
                ...
                onPressed: () => _run(() => _preferences.removeExperiment(experiment.id)),
                icon: const Icon(Icons.delete_outline_rounded),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Text(result.summary, style: const TextStyle(color: Colors.white70, height: 1.4)),
          const SizedBox(height: 10),
          Wrap(
            spacing: 8,
            runSpacing: 8,
  ...
  Future<void> _pickWorkoutFile() async {
    ...
    try {
      ...
      setState(() {
        _message = '${parsed.workouts.length} workout${parsed.workouts.length == 1 ? '' : 's'} imported from ${file.name}.';
      });
  ...
  Future<void> _newExperiment() async {
    final template = await showModalBottomSheet<LabExperimentTemplate>(
      ...
      builder: (context) => SafeArea(
        child: ListView(
          ...
          children: <Widget>[
            const ListTile(
              title: Text('START AN EXPERIMENT',
                  style: TextStyle(fontWeight: FontWeight.w900)),
    ...
    final experiment = switch (template) {
      LabExperimentTemplate.caffeineTiming => LabExperimentTemplates.caffeineTiming(),
      LabExperimentTemplate.creatineConsistency =>
        LabExperimentTemplates.creatineConsistency(),
      LabExperimentTemplate.preWorkoutMealTiming => LabExperimentTemplates.mealTiming(),
  ...
  Widget _privacySwitch(String title, bool value, ValueChanged<bool> onChanged) =>
      SwitchListTile.adaptive(
        contentPadding: EdgeInsets.zero,
        dense: true,
        title: Text(title),
...
class _StatusCard extends StatelessWidget {
  const _StatusCard({required this.title, required this.detail, required this.positive});
...
class _Notice extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
        ...
        child: SafeArea(
          bottom: false,
          child: ListTile(
            leading: const Icon(Icons.info_outline_rounded, color: Color(0xff22d3ee)),
            title: Text(message),
            trailing: IconButton(onPressed: onClose, icon: const Icon(Icons.close_rounded)),
          ),
        ),
      );
}
